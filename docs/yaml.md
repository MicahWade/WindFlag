...
*   **`category`** (string, optional): The name of the category the challenge belongs to. If a category with this name already exists, the challenge will be assigned to it. If it does not exist, a new category (with `NONE` unlock type) will be automatically created upon import. Defaults to "Uncategorized".
